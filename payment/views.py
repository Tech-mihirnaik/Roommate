...
def Pay1(request):
    bookout=bookingrooms.objects.get(email=request.session['pay_email'],gender=request.session['gender'],hostel_name=request.session['hostel_name'],category=request.session['category'],rejected=False)
    id_new="_"+bookout.booking_id[1:]
    current_site = get_current_site(request)
    param_dict = {
        'MID': 'QZTXpY23892438318004',
        'ORDER_ID': str(id_new),
...
def handlerequest(request):
    ...
    if verify:
        if response_dict['RESPCODE'] == '01':
            ...
            pl=payment_link.objects.get(orderid="#"+str(response_dict['ORDERID']))
            pl.delete()
            ar=addroom.objects.filter(hostel_name=room.hostel_name,category=room.category,gender=room.gender)[0]
            ar.num_room= str(int(ar.num_room)+1)
            ar.save()
...
def handlerequest2(request):
    ...
    print(response_dict)
    success=False
    # verify = Checksum.verify_checksum(response_dict, "xjvVNldBjXF1@NcP", checksum)
    verify = Checksum.verify_checksum(response_dict, "RIzZj7u_HfAKscS8", checksum)
    
    id="#"+(str(response_dict['ORDERID']))
    m=bookedCab.objects.get(booking_id=id).email
    if verify:
        if response_dict['RESPCODE'] == '01':
            success=True
            bc=bookedCab.objects.get(booking_id=id)
            bc.payment=True
            bc.transaction_id_200=response_dict['TXNID']
            bc.payment_mode_200=response_dict['PAYMENTMODE']
            bc.transaction_date_200=response_dict['TXNDATE']
            bc.bank_transaction_id_200=response_dict['BANKTXNID']
            bc.bank_name_200=response_dict['GATEWAYNAME']
            bc.save()
            print(id)
            print('order successful')
            send_mailcab(request,id)
            send_mailstud(request,bc.email)
            
        else:
            # send_mailfail_cab(request,id)
            # send_mailTest(request,m)
            bookedCab.objects.get(booking_id=id).delete()
            print('order was not successful because' + response_dict['RESPMSG'])
    return render(request, 'payment/success_cab.html', {'response': response_dict,'success':success,'id':id})



def send_mailcab(request,id):
    email_subject = 'CAB BOOKED'
    br=bookedCab.objects.get(booking_id=id)
    ...
    email_body = strip_tags(html_message)
    msg = EmailMultiAlternatives(email_subject, email_body, settings.EMAIL_FROM_USER, ['techmihirnaik@gmail.com'])
    # msg = EmailMultiAlternatives(email_subject, email_body, settings.EMAIL_FROM_USER, ['joshishivansh28012001@gmail.com'])
    msg.attach_alternative(html_message, "text/html")
    msg.send()
