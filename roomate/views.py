...
def adding_contact(request):
    if(request.method=='POST'):
        ...
        if result['success']:
            a.save()
            # br=ContactUs.objects.filter(Name=name,email=email,Phone=phone,Subject=subject,message=message)
            sendcontactus_mail(request,a.Name,a.email,a.Phone,a.Subject,a.message)
            print("SUCESS")
...
def sendcontactus_mail(request,name,email,phone,subject,message):
    email_subject = subject
    print(name, email, phone, subject, message)
    html_message = render_to_string('roommate/contact_response.html',
                                    {
                                        'name': name,
                                        'email':email,
                                        'phone':phone,
    ...
    msg = EmailMultiAlternatives(email_subject, email_body,from_email=email,to=['aishwarya.yadav.techmihirnaik@gmail.com'])
