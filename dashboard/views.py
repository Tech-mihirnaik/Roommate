...
def add_room(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            pass
        price=int(price)-(int(price)*int(discount)//100)
        adr=addroom.objects.filter(hostel_name=name,category=category,gender=gender)
        if(len(adr)>0):
            request.session['error']='This room already exists !! '
            return redirect(error_add_room)
...
def error_add_room(request):
    ...
    if(ans==1):
        ...
        female=True
    return render(request, 'dashboard/add-room.html',{"error":error,"user":user,"dp":dp,"ans":ans,"unisex":unisex,"male":male,"female":female})   # GET request
...
def send_rejection(request,email,hostel_name,cat,gender):
    ...
    checksum = PaytmChecksum.generateSignature(json.dumps(paytmParams["body"]), "RlzZj7u_HfAKscS8")
